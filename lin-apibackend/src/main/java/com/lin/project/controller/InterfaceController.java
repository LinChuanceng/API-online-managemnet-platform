...
import com.lin.api.model.entity.User;
...
import com.lin.project.model.enums.InterfaceInfoEnum;
...
//校验接口是否存在
        if (idRequest.equals(" ") || idRequest==null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        ...
        String username = linApiClient.getUsernameByPost(user);
    ...
    @PostMapping("/offline")
    @AuthCheck(mustRole = "admin")
    public BaseResponse<Boolean> offlineInterfaceInfo(@io.swagger.v3.oas.annotations.parameters.RequestBody IDRequest idRequest){
        if (idRequest.equals(" ") || idRequest==null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        long id = idRequest.getId();
