...
-- 接口信息
create table if not exists linapi.`interface_info`
(
...
insert into linapi.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                     `method`, `userId`)
...
-- 用户调用接口关系表
create table if not exists linapi.`user_interface_info`
(
    `id`              bigint                             not null auto_increment comment '主键' primary key,
